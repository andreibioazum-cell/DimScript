...
def compile_ds(input_file):
    ...
    # Заголовок рантайма (встроен прямо в компилятор для скорости)
    cpp = [
    ...
    for l in lines:
        ...
        if l.startswith('func '):
            ...
            if m: l = f"{m.group(2).replace('i32','int')} {m.group(1)}{m.group(3)}"
    ...
    # Точка входа в машинный код Android
    cpp.append('extern "C" void ANativeActivity_onCreate(void* a, void* b, size_t c) { dim::main_logic(); }')
